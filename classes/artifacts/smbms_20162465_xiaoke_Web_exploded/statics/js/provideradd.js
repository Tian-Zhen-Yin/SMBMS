...
$(function(){
	...
	proName.on("focus",function(){
		...
		if(proName.val() != null && proName.val() != ""){
			$.ajax({
				type:"GET",//请求类型
				url:path+"/provider/checkProviderName",//请求的url
				data:{proName:proName.val()},//请求参数
				dataType:"json",//ajax接口（请求url）返回的数据类型
				success:function(data){//data：返回数据（json对象）
					if(data== "exist"){//账号已存在，错误提示
						validateTip(proName.next(),{"color":"red"},imgNo+ " 该用供应商名称已存在",false);
					}else if(data=="noExist"){//账号可用，正确提示

...
function deleteProvider(obj){
	$.ajax({
		type:"GET",
		url:path+"/provider/deleteProvider",
...
$(function(){
	$(".viewProvider").on("click",function(){
		...
		var obj = $(this);
		window.location.href=path+"/provider/findById?uid="+ obj.attr("proid");
	});
	
	$(".modifyProvider").on("click",function(){
		var obj = $(this);
		window.location.href=path+"/provider/toModify?proid="+ obj.attr("proid");
	});
